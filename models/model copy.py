...
from models.VQVAE_Adapter import VQVAE_Adapter
# from models.linear_vqvae_adapter import LinearVQVAE_Adapter as VQVAE_Adapter # 新的 Linear 版
...
class ClientEncoder(nn.Module):
    ...
    def forward(self, info, x_inp, mask):
        ...
        n_vars = x_inp.shape[1]

        if self.use_vq:
            # VQVAE_Adapter 期望输入 [B, Seq_Len, 1] 或 [B, Seq_Len, C]
            # 当前 x_inp 是 [B, C, L]，我们需要转为 [B, L, C]
            x_for_vq = x_inp.permute(0, 2, 1) # [Batch, Seq_Len, Channels]
            
            # 获取量化特征和 Loss
            # z_q: [Batch*Channels, Num_Patches, D] (如果 adapter 内部做了展平)
            z_q, vq_loss = self.vqvae(x_for_vq)
            
            # 调整形状以适配后续 ReprogrammingLayer
            # 此时 z_q 已经是 Token Embedding 格式，直接作为 x_token
            # 注意：Adapter 输出通常是 [Batch*Channels, Num_Patches, D]
            # Time-FFM 原逻辑期望 x_token 为 [Batch*Channels, Num_Patches, D]
            x_token = z_q 
        else:
            # [原有] 线性 Patching
